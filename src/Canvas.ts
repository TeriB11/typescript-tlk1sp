...
export class Canvas {
  ...
  constructor(
    ...
    options: {
      size:
        | { autoresize: false; canvasSize: Vec2 }
        | { autoresize: true; maxCanvasWidth: number };
    }
  ) {
    ...
    if (options.size.autoresize === true) {
      const maxWidth = options.size.maxCanvasWidth;
      this._canvasSize = this.autoresizeSize(maxWidth);
      window.addEventListener('resize', () => {
        this.canvasSize = this.autoresizeSize(maxWidth);
        this.update();
      });
    } else {
      this._canvasSize = options.size.canvasSize;
    }
    this.updateRects();
  ...
  private autoresizeSize(maxWidth: number) {
    const width = this.containerElement.clientWidth;
    const newWidth = maxWidth <= 0 ? width : Math.min(width, maxWidth);
    return new Vec2(newWidth, newWidth);
  }
  ...
  private updateRects() {
    const { canvas } = this;
    this._viewRect = Rect.create(0, 0, ...this.canvasSize.components);
  ...
  onUpdate: (canvas: Canvas) => void | undefined = undefined;
  ...
  renderInViewport(
    ...
    const canvasRect =
      options.canvasRect ?? Rect.create(0, 0, ...this.canvasSize.components);

    this.context.beginPath();
    this.context.rect(...canvasRect.components);
    this.context.clip();

    const mat = Matrix.viewportTransformation(options.viewport, canvasRect);
  ...
  setRenderLoop(
    ...
    window.clearInterval(this._renderLoopId);

    if (shouldRun) {
      const startTime = Date.now();
      let lastTime = startTime;

      this._renderLoopId = window.setInterval(() => {
